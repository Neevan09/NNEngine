...
Implementation * implementationInstance = nullptr;
...
{
	auto foundIt = implementationInstance->audioSounds.find(soundName);			// Find sound  from the list of loaded files (cache)
	if (foundIt != implementationInstance->audioSounds.end())
		return;
	...
	int error = AudioEngine::ErrorCheck(implementationInstance->audioSystem->createSound(soundName.c_str(), mode, nullptr, &currentSound));
	if (error == 1)
	{
	...
	if (currentSound)
	{
		implementationInstance->audioSounds[soundName] = currentSound;
	}
}
...
void AudioEngine::UnLoadSound(const string& soundName)
{
	...
	implementationInstance->audioSounds.erase(foundIt);
}
...
void AudioEngine::Set3dListenerAndOrientation(const AudioVector3 & position, const AudioVector3 & look, const AudioVector3 & up)
{
	...
	AudioEngine::ErrorCheck(implementationInstance->audioSystem->set3DListenerAttributes(0,&pos,&velocity,&forward,&updir));
}
...
{
	int nxtChannelId = implementationInstance->nextChannelId++;
	auto foundIt = implementationInstance->audioSounds.find(soundName);			// Find sound  from the list of loaded files
	if (foundIt == implementationInstance->audioSounds.end())					// If sond not found then try and load sound
	{
		LoadSound(soundName);
		foundIt = implementationInstance->audioSounds.find(soundName);
		if (foundIt == implementationInstance->audioSounds.end())				// Coundn't load sound hence cannot play osund
		{
			return nxtChannelId;
		}
	}
	...
	AudioEngine::ErrorCheck(implementationInstance->audioSystem->playSound(foundIt->second, nullptr, true, &currentChannel));	// pause on startup to avoid cutting
	...
	if (currentChannel)
	{
		...
		implementationInstance->audioChannels[nxtChannelId] = currentChannel;						// Update Channel cache
		this->FadeInChannel(nxtChannelId, fadeInTime, DecibalToVolume(volume));						// Fade the channel into a certain volume level
		implementationInstance->audioSoundChannels[soundName] = currentChannel;						// Map sound name to channel
...
{
	auto foundIt = implementationInstance->audioChannels.find(channelId);
	if (foundIt == implementationInstance->audioChannels.end())
	{
		return;
	}
...
{
	bool playing;
	for (auto channelMap : implementationInstance->audioChannels)
	{
		channelMap.second->isPlaying(&playing);
		if(playing)
			AudioEngine::ErrorCheck(channelMap.second->stop());
	}
	//implementationInstance->nextChannelId = 0;
}
...
void AudioEngine::StopChannel(int channelId,float fadeOutTime)
{
	...
	//implementationInstance->nextChannelId--;
}
...
{
	auto foundIt = implementationInstance->audioBanks.find(strBankName);
	if (foundIt != implementationInstance->audioBanks.end())
		return;
	
	FMOD::Studio::Bank* currentBank;
	AudioEngine::ErrorCheck( implementationInstance->mpStudioSystem->loadBankFile(strBankName.c_str(), flags, &currentBank));
	if (currentBank)
	{
		implementationInstance->audioBanks[strBankName] = currentBank;
	}
}
...
{
	auto foundit = implementationInstance->audioEvents.find(eventName);
	if (foundit != implementationInstance->audioEvents.end())
		return;
	FMOD::Studio::EventDescription* eventDescription = NULL;
	AudioEngine::ErrorCheck(implementationInstance->mpStudioSystem->getEvent(eventName.c_str(), &eventDescription));
	if (eventDescription)
	{
		...
		if (eventInstance)
		{
			implementationInstance->audioEvents[eventName] = eventInstance;
		}
	}
}
...
void AudioEngine::PlayEvents(const string &eventName)
{
	...
	if (foundit == implementationInstance->audioEvents.end())
	{
		...
		foundit = implementationInstance->audioEvents.find(eventName);
		if (foundit == implementationInstance->audioEvents.end())
			return;
	}
...
{
	AudioEngine::ErrorCheck(implementationInstance->reverb->setActive(active));
}
...
{
	bool active;
	AudioEngine::ErrorCheck(implementationInstance->reverb->getActive(&active));
	return active;
}
...
{
	AudioEngine::ErrorCheck(implementationInstance->audioSystem->createReverb3D(&implementationInstance->reverb));
	implementationInstance->reverb->setProperties(&reverbProperties);
	FMOD_VECTOR pos = VectorToFmod(position);
	float mindist = minDist;
	float maxdist = maxDist;
	AudioEngine::ErrorCheck(implementationInstance->reverb->set3DAttributes(&pos, mindist, maxdist));
}
